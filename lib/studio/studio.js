...
function show(view) {
  ...
  if (view === 'bench') drawBenchHead().catch((e) => toast(e.message, true));
...
async function loadContext() {
  ...
  $('runBench').disabled = !ctx.capabilities.bench || benchRunning;
}
...
function drawHeader() {
  ...
  for (const b of $('targets').querySelectorAll('button')) b.onclick = () => {
    ...
    drawHeader(); drawOverview(); if (current && current.findings) drawInspector(current, !$('liveBadge').hidden); drawBenchHarnesses();
  };
}
...
// Opening another artifact (or a new one) drops a check still pending for the previous one.
function cancelLiveCheck() { clearTimeout(checkTimer); checkSeq++; }
function onEdit() {
...
async function liveCheck() {
  ...
  const seq = ++checkSeq, forPath = current.path;
  try {
    const r = await backend.check(editPayload());
    if (seq !== checkSeq || !current || current.path !== forPath) return;   // a newer edit, or another artifact
  ...
  } catch (e) { if (seq === checkSeq) { $('editHint').textContent = e.message; $('editHint').className = 'hint warn'; } }
}
...
function findingCard(f, i) {
  ...
  const where = line ? ' <button class="loc" data-line="' + line + '" title="show in the editor">line ' + line + '</button>' : '';
  ...
  return '<div class="find ' + esc(f.severity) + '">' +
    ...
    '<div class="actions">' +
      (f.autofix ? '<button class="btn small fixbtn" data-i="' + i + '"' + (ro ? ' disabled title="read-only"' : '') + '>' + esc(f.autofix.label) + '</button>' + safety
        : '<span class="manual">No automatic fix — ' + (line ? 'edit <button class="loc" data-line="' + line + '">line ' + line + '</button> by hand' : 'edit the artifact by hand') + '.</span>') +
...
async function openSuppress(f) {
  ...
  $('modalReason').value = '';
  let preview = null, timer = null, gen = -1;
  ...
  const refresh = async () => {
    if (gen !== modalGen) return;                              // this modal was closed or replaced
    ...
    try {
      const r = await api('/api/suppress', payload());
      if (gen !== modalGen) return;
      preview = r; $('modalDiff').innerHTML = diffHtml(r.diff); $('modalApply').disabled = false; $('modalNote').textContent = 'Written to ' + r.file + '. The finding stays visible here, marked suppressed, and no longer counts.';
    } catch (e) { if (gen === modalGen) $('modalDiff').textContent = e.message; }
  };
...
// saut.json changed, the artifact did not: re-lint what the editor holds — an unsaved edit
// stays in the editor (re-linted as it is), a clean one is re-read from disk.
async function relint() {
  if (dirty) { cancelLiveCheck(); await liveCheck(); }
  else await openArtifact(current.path);
}
...
function drawReviewSetup() {
  ...
  $('runReview').disabled = !on || !(current && current.path) || reviewRunning;
...
$('runReview').onclick = async () => {
  ...
  try {
    ...
    openModal('Review ' + current.name + ' with ' + e.model,
      ...
      '', async () => { closeModal(); runReview(model, text); }, 'Run the review');   // it reports its own outcome
  } catch (err) { toast(err.message, true); }
};

let reviewRunning = false;
async function runReview(model, text) {
  const forPath = current.path, name = current.name;
  $('secReview').open = true;
  $('reviewOut').dataset.for = forPath;
  $('reviewOut').innerHTML = '<div class="muted">Reviewing… one model call, usually under a minute.</div>';
  reviewRunning = true; $('runReview').disabled = true;
  const here = () => current && current.path === forPath;
  try {
    const r = await api('/api/review', { path: forPath, model, text });
    if (here()) drawReview(r);
    else toast('the review of ' + name + ' finished — it is kept under ~/.saut/results; open ' + name + ' and run it again to apply it');
  } catch (e) {
    if (here()) $('reviewOut').innerHTML = '<div class="bad">' + esc(e.message) + '</div>';
    else toast('the review of ' + name + ' failed: ' + e.message, true);
  } finally { reviewRunning = false; if (!here()) delete $('reviewOut').dataset.for; drawReviewSetup(); }
}
...
async function applyProposal(c, button) {
  if (!current || $('reviewOut').dataset.for !== current.path) return toast('this proposal is for another artifact', true);
  try {
    if (editMode !== 'source') await document.querySelector('.subtab[data-edit=source]').onclick();
    if (editMode !== 'source' || $('reviewOut').dataset.for !== current.path) return;
    const text = textOf(sourceView);
...
let pending = null, modalGen = 0, modalBusy = false, modalReturn = null;
...
function openModal(title, note, diff, apply, applyLabel, disabled) {
  modalGen++;
  if (!modalOpen()) modalReturn = document.activeElement;
  pending = apply;
...
// While the dialog is open the page behind it takes no focus and no typing — an edit made
// there would be lost when the dialog's action re-reads the file.
function setInert(on) { for (const el of document.body.children) if (el.id !== 'modal' && el.id !== 'toast' && el.tagName !== 'SCRIPT') el.inert = on; }
function closeModal() {
  modalGen++;
  $('modal').style.display = 'none'; pending = null; $('modalForm').hidden = true;
  setInert(false);
  const back = modalReturn; modalReturn = null;
  if (back && back.isConnected && typeof back.focus === 'function') back.focus();
}
$('modalCancel').onclick = () => { if (!modalBusy) closeModal(); };
$('modal').onclick = (e) => { if (e.target === $('modal') && !modalBusy) closeModal(); };
$('modalApply').onclick = async () => {
  const run = pending;
  if (!run || modalBusy) return;
  const gen = modalGen;
  modalBusy = true; $('modalApply').disabled = true;
  try { await run(); if (gen === modalGen) closeModal(); }
  catch (e) { toast(e.message, true); if (gen === modalGen) $('modalApply').disabled = false; }
...
let saving = false;
async function save() {
  if (!current || current.readOnly || saving || modalOpen()) return;
  saving = true;
  try {
    if (current.isNew) {
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && modalOpen()) return modalBusy ? undefined : closeModal();
  if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 's' && !$('view-skill').hidden) { e.preventDefault(); if (!modalOpen()) save(); }
});

// ── bench ─────────────────────────────────────────────────────────
// A run, a before/after pair, the cases, the history and a comparison — all for the artifact
// open in the Skill view.
let benchCases = null, benchRuns = [], selectedRuns = [], estimateTimer = null, benchRunning = false;
...
async function drawBenchHead() {
  ...
  $('runBench').disabled = !ctx.capabilities.bench || !has || benchRunning;
  drawBenchHarnesses();
  updateMeasure();
  benchCases = null;
...
function updateMeasure() {
  ...
  $('measureBench').disabled = !can || benchRunning;
...
function streamBench(id, onEnd) {
  ...
  es.onerror = () => { es.close(); onEnd({ error: 'lost the event stream — the run may still be going; its result lands in History' }); };
}
function benchBusy(on) {
  benchRunning = on;
  $('runBench').disabled = on || !ctx.capabilities.bench || !(current && current.path);
  $('measureBench').disabled = on; if (!on) updateMeasure();
}

$('runBench').onclick = async () => {
  if (!current || !current.path) return toast('open an artifact first', true);
  if (benchRunning) return;
  const forPath = current.path;
  ...
  try {
    ...
    streamBench(id, async ({ result, error }) => {
      ...
      if (!current || current.path !== forPath) return toast('the run of ' + forPath + ' finished — it is in that artifact’s History');
      if (result) drawBenchMatrix(result);
      await loadRuns().catch((e) => toast(e.message, true)); scheduleEstimate();
    });
  } catch (e) { benchBusy(false); toast(e.message, true); }
};

$('measureBench').onclick = async () => {
  if (!current || !current.path || !dirty || benchRunning) return;
  const forPath = current.path;
  benchBusy(true);
  try {
    const c = await backend.compose(editPayload());
    if (!c.ok) { benchBusy(false); return toast(c.reason, true); }
    if (!c.diff) { benchBusy(false); return toast('the edit changes nothing — there is nothing to measure'); }
    $('benchLog').hidden = false; $('benchLog').textContent = ''; $('benchMatrix').innerHTML = '';
    const { id } = await backend.bench(benchPayload({ variantText: c.text }));
    streamBench(id, async ({ pair, error }) => {
      benchBusy(false);
      if (error) return toast(error, true);
      if (!current || current.path !== forPath) return toast('the measurement of ' + forPath + ' finished — it is in that artifact’s History');
      await loadRuns().catch((e) => toast(e.message, true)); scheduleEstimate();
...
async function loadCases() {
  const forPath = current.path;
  const got = await api('/api/cases?path=' + encodeURIComponent(forPath));
  if (!current || current.path !== forPath) return;             // another artifact was opened meanwhile
  benchCases = got;
...
$('newCase').onclick = () => { if (benchCases) editCase(null); };
...
$('writeAuto').onclick = async () => {
  if (!benchCases || !current || !current.path) return;
  try {
...
async function loadRuns() {
  const forPath = current.path;
  const got = (await api('/api/runs?path=' + encodeURIComponent(forPath))).runs;
  if (!current || current.path !== forPath) return;
  benchRuns = got;
