...
test('catalog: every code the sources can emit is explained, and RULES.md documents the same set', async () => {
  const src = (await Promise.all(['lint.mts', 'frontmatter.mts', 'skill.mts', 'adapters/taut.mts', 'guidance.mts', 'suppress.mts'].map((f) => fs.readFile(path.join(REPO, 'lib', f), 'utf8')))).join('\n');
